...
class SessionPollAdapter(BotAdapter):
    async def send_prompt(self, prompt: str, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        http_timeout = config.get("timeout_ms", 30000) / 1000
        ...
        deadline = start + poll.get("timeout_ms", 30000) / 1000
        ...
        return self._fail(f"no agent reply within {poll.get('timeout_ms',30000)}ms", start, conv=conv)
