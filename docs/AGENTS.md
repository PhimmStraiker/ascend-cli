...
The CLI is the API. Everything below exists so an agent — Claude Code in a terminal, a CI job, a
bot — can read state, act, and recover without parsing English.
...
`1` vs `2` is deliberate: "the target is unreachable" and "the target answered badly" require
different responses.
...
Without `--if-not-exists`, a retried create leaves **two apps with the same name** — which makes
...
This is the most important thing to understand before using `ascend results` in an automated
pipeline, because it determines what you can trust and what you must still judge.
...
deliberately outside the CLI's code. Point an agent at that file plus `ascend results --json`.

## Reading turn-level results

`ascend results <file>` takes a Console CSV export or a local capture and sniffs which it is.
...
Two units traps, both of which produce confidently wrong statements if missed:
...
2. **Unanswered probes are not passes.** On real exports this is routinely 30–43% of a run. The
   failure rate is computed over `answered` for exactly this reason; if `unanswered` is high, the
   honest conclusion is that the run under-measured, not that the target did well.

## Things that will bite an agent if you don't know them
...
1. **A run with no bridge still completes — and looks clean.** Unanswered probes are not findings, so
   ...
   reconcile. This stays the single most important check in the whole tool.
...
4. **One tenant per machine.** The CLI pins itself to the first tenant it sees and refuses another
   PAT (`ascend tenant show`). This is intentional: it stops customer material from crossing.
5. **`tc-` keys are shown once.** They're stored automatically by `app create`/`onboard`; if a
   create response lacks one, the command fails loudly rather than storing nothing.
6. **Long calls are slow, not stuck.** There is no tenant-wide assessments endpoint, so anything
...
8. **`controls validate` exits 3 on an unknown id.** Deliberately: a control that does not exist
...
For an agent with shell access the CLI is strictly better: no install or config step, and it
composes — pipe `--json` into `jq`/`python`, tail a bridge log, chain steps in one command. An MCP
layer only earns its place for clients that *can't* run a shell, or when you deliberately want to
restrict an agent to a fixed set of verbs. A thin MCP passthrough exists in `shells/mcp/` for that
case.
